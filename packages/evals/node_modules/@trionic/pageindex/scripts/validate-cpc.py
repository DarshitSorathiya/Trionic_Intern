...
CPC_TREE_FILE = ROOT / "packages" / "pageindex" / "artifacts" / "cpc-tree.json"
VALIDATION_REPORT_FILE = ROOT / "packages" / "pageindex" / "reports" / "cpc-validation.json"

def main():
    print("Starting CPC tree validation...")

    if not CPC_TREE_FILE.exists():
        print(f"Error: Tree file not found at {CPC_TREE_FILE}")
        import sys
        sys.exit(1)

    with open(CPC_TREE_FILE, "r", encoding="utf-8") as f:
        nodes = json.load(f)
    ...
    # Formats to check
    id_patterns = {
        "act": re.compile(r"^CPC-1908$"),
        "chapter": re.compile(r"^CPC-1908/ORD-[IVXLCDM]+(?:\s*[A-Z]|-A)?$"),
        "section": re.compile(r"^(?:CPC-1908/S-\d+[A-Z]?|CPC-1908/ORD-[IVXLCDM]+(?:\s*[A-Z]|-A)?/R-\d+[A-Z]?)$")
    }
    ...
    substantive_sections = [n for n in nodes if n["type"] == "section" and n["id"].startswith("CPC-1908/S-")]
    print(f"\nChecking 5 random substantive sections out of {len(substantive_sections)} against IndiaCode ground truth:")
    random.seed(42) # Set seed for reproducible validation run
    sampled_sections = random.sample(substantive_sections, min(len(substantive_sections), 5))

    # Fetch source IndiaCode JSON
    url = "https://raw.githubusercontent.com/civictech-India/Indian-Law-Penal-Code-Json/master/cpc.json"
    source_data = []
    try:
    ...
    if source_data:
        for sec in sampled_sections:
            ...
            source_item = next((item for item in source_data if str(item.get("section")).strip().rstrip('.') == sec_num_str), None)
            if not source_item:
                errors.append(f"Section {sec['id']} (number {sec_num_str}) not found in IndiaCode source data")
                continue
            
            source_title = source_item.get("title", "").strip()
            ...
            # In tree, title has 'Section <num> — <Title>' format
            clean_source_title_full = f"section {sec_num_str} — {' '.join(source_title.split()).strip()}"
            clean_source_title_full = clean_source_title_full.lower()
            
            # Allow substring check or direct match
            if clean_source_title_full not in clean_tree_title and clean_tree_title not in clean_source_title_full:
